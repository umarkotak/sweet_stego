...
class Sha512 {
    ...
    static hash(msg) {
        ...
        msg = utf8Encode(msg);
        console.log('utf8Encoded message : ', msg);
        ...
        console.log('first 32 bits of the fractional parts of the cube roots of the first 80 primes');
        ...
        console.log('SHA-512 initial hash values (in big-endian)');
        console.log('const H : ', H);

        // PREPROCESSING [§6.4.1]

        msg += String.fromCharCode(0x80);  // add trailing '1' bit (+ 0's padding) to string [§5.1.2]
        console.log('hexadecimal of 80 => 1000 0000');
        console.log('message add trailing 1 bit : ', msg);
        ...
        console.log('number of 16-long blocks required to hold length ints : ', N);
        console.log('initiate message Nx16 array of 64-bit integers : ', M);
        ...
        console.log('completed message Nx16 array of 64-bit integers : ', M);
        ...
        console.log('M after penambahan final pair [N-1][14] : ', M[N-1][14]);
        ...
        console.log('M after penambahan final pair [N-1][15] : ', M[N-1][15]);
        ...
        for (let i=0; i<N; i++) {
            ...
            console.log('Message schedule (W) : ', W);

            for (let t=16; t<80; t++) {
                W[t] = (Sha512.σ1(W[t-2]).add(W[t-7]).add(Sha512.σ0(W[t-15])).add(W[t-16]));
            }
            console.log('W setelah dilakukan rotasi : ', W);
            ...
            for (let i = 0; i <= 7; i++) console.log('intermediate new H ', i, ' value : ', H[i]);
        }

        // convert H0..H7 to hex strings (with leading zeros)
        for (let h=0; h<H.length; h++) H[h] = H[h].toString();
        console.log('Konversi H kedalam bentuk string dengan angka 0 : ', H);

        // concatenate H0..H7, with separator if required
        const separator = '';

        var chipper_text = H.join(separator);
        console.log('Hash val : ', gethashval(chipper_text));
...
Sha512.Long = class {
    ...
    // partial convertion hi and lo hexadecimal to decimal
    ...
    // convert from 64-bit integer to constant
    toString() {
...
function gethashval(pass) {
  var result = 0;
  var imgdatalength = 200;
