...
function extractMsgArray_pass(bitarray, pass) {
  ...
  var msgarray = Array();
  var data;
  var msgarraylen = Math.floor(imgdatalength / 8);
  for (var i = 0; i < msgarraylen; i++) {
    ...
    for (var j = 0; j < 8; j++) {
      var tmpArray = Array();
      tmpArray.push(bitarray[gethashval(pass, imgdatalength, taken)]);
      pass = String(CryptoJS.SHA512(pass));
...
function bitconvert(str) {
  ...
  for (var i = 0; i < utf8strlen; i++) {
    var message_data = i + ". ";
  	message_data += str[i] + " => " + utf8array[i] + " => ";
    ...
    message_data += result.slice(bil, bil+8).map( x => +x ) + " => ";
    message_data += result.slice(bil, bil+8) + "\n";
    message_tmp += message_data
    bil += 8;
  }
  $("#log_before_message_in_binary").val(message_tmp);
  return result;
}
...
function generate_pass(image_data_length, message, pass) {
  ...
  $('#log_after_valid_pixels').val(taken.join(', '));
  $('#log_after_valid_pixels_asc').val(taken.sort(function(a, b){return a-b}).join(', '));
...
function writeMsgToCanvas(canvasid, message, pass) {
  ...
  var image_spesification = "";
  var image_data = "";
  var no = 1;
  var channel_no = 0;
  image_spesification += "lebar gambar = " + c.width + " pixels | tinggi gambar = " + c.height + " pixels \n";
  image_spesification += "kapasitas penyimpanan gambar = " + Math.floor(imageData.data.length / 4) * 3 + " bits \n";
  image_spesification += "data yang dipakai untuk penyimpanan gambar = " + Math.floor(imageData_clean.data.length / 4) * 3 + "bits \n";
  for (var i = 0; i < imageData_clean.data.length; i+= 4) {
    var R = imageData_clean.data[i]; var G = imageData_clean.data[i+1]; var B = imageData_clean.data[i+2]; var A = imageData_clean.data[i+3];
    image_data += "pixel - " + no + " [" + channel_no + ", " + (channel_no+1) + ", " + (channel_no+2) + "] = RGB(" + R + ", " + G + ", " + B + ") => ";
    image_data += dectobin(R) + ", " + dectobin(G) + ", " + dectobin(B) + "\n";
    no++; channel_no+=3;
  }
  $('#log_before_image_spesification').val(image_spesification);
  $('#log_before_image_data').val(image_data);
  ...
  $('#log_after_image_data').val(image_data);
  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = function(e) {
      ...
      image.onload = function() {
        var width = parseInt(($('#certificate_custom_width').val())) || 800;
        var height = parseInt($('#certificate_custom_height').val()) || 450;
        var height_data = parseInt($('#certificate_custom_data_offset').val()) || 10;
        console.log("IMAGE CONFIG:", width, height, height_data)
