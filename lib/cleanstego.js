...
function readMsgFromCanvas_single(canvasid, pass) {
  ...
  var msgArray = extractMsgArray_pass(bitarray, pass, 1);
...
function generate_pass(image_data_length, message, pass) {
  ...
  for (var i = 0; i < message_binnary_block.length; i++) {
    result[gethashval(pass, image_data_length, taken)] = message_binnary_block[i];
    pass = String(CryptoJS.SHA512(pass));
  }
  for (var j = 0; j < 24; j++) {
    result[gethashval(pass, image_data_length, taken)] = true;
    pass = String(CryptoJS.SHA512(pass));
  }

  console.log('========= MESSAGE SPESIFICATION =========');
  console.log('Message size : ', message_binnary_block.length + 24);
  console.log('Message in binary : ', message_binnary_block);
  console.log('Pass : ', pass);
  console.log('Result obfuscation : ', result);
  var true_val = Array();
...
//Write message to the image in canvas id.
//Return: null - fail. 1 - successful
function writeMsgToCanvas_single(canvasid, message, pass, dct, copy, lim) {
  pass = (pass === undefined) ? '' : pass;
  ...
  var imageDataLength = Math.floor(imageData.data.length / 4) * 3;
  var setdata = generate_pass(imageDataLength, message, pass);
  ...
  console.log('True index map : ', true_val);
...
//Read message from the image in canvas id.
...
function loadIMGtoCanvas(inputid, canvasid, callback, maxsize, heightdata) {
  ...
  if (input.files && input.files[0]) {
    ...
    reader.onload = function(e) {
      ...
      image.onload = function() {
        ...
        canvas_clean.width = w;
        canvas_clean.height = heightdata;
        canvas_clean.style.display = "none";
        var body = document.getElementsByTagName("body")[0];
        body.appendChild(canvas_clean);
        var context_clean = canvas_clean.getContext('2d');
        context_clean.drawImage(image, 0, 0, image.width, heightdata, 0, 0, w, heightdata);
