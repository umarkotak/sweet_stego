...
function initialize_obfuscation(length) {
  result = Array();
...
function generate_pass(imgdatalength, information, pass, copy) {
  ...
  if ((info.length + 24) > imgdatalength) {
    alert('Pesan yang anda masukkan terlalu panjang');
    return null;
  }
  var result = initialize_obfuscation(imgdatalength);
...
function writeMsgToCanvas_single(canvasid, msg, pass, dct, copy, lim) {
  ...
  var ctx = c.getContext("2d");
  var imgData = ctx.getImageData(0, 0, c.width, c.height);
  var datalength = Math.floor(imgData.data.length / 4) * 3;
  var setarray = generate_pass(datalength, msg, pass, 1);
